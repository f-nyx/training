...
public class TtyEmulator {
  private static final Logger LOG = LoggerFactory.getLogger(TtyEmulator.class);
  ...
  private final PtyTerminal myTerminal;
  private boolean myEof = false;

  public TtyEmulator(TtyConnector connector, PtyTerminal terminal) {
    myDataStream = new TerminalDataStream(connector);
    myTerminal = terminal;
  ...
  public void moveCursor(int x, int y) {
    myTerminal.cursorPosition(x, y);
  }

  public void lock() {
    myTerminal.buffer().lock();
  }

  public void unlock() {
    myTerminal.buffer().unlock();
  }
  ...
  public void processChar(char ch, PtyTerminal terminal) throws IOException {
    switch (ch) {
      case 0:
        break;
  ...
  private void processEscapeSequence(char ch, PtyTerminal terminal) throws IOException {
    switch (ch) {
  ...
  private void processTwoCharSequence(char ch, PtyTerminal terminal) throws IOException {
